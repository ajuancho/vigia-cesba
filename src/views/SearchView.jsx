...
import { NORMAS_FEED, TIPOS_NORMA, SECTORES, JURISDICCIONES } from '../data/mockData';
...
const TIPO_DOT_COLORS = {
    ...
    RESOLUCION: '#2563eb', DISPOSICION: '#7c3aed', PROYECTO: '#0891b2',
};
...
const SearchView = () => {
    ...
    const results = NORMAS_FEED.filter(n => {
        const matchQuery = !query ||
            n.titulo.toLowerCase().includes(query.toLowerCase()) ||
            n.resumen.toLowerCase().includes(query.toLowerCase()) ||
            n.tags.some(t => t.toLowerCase().includes(query.toLowerCase())) ||
            n.entidades.some(e => e.toLowerCase().includes(query.toLowerCase()));
        const matchTipo = !tipo || n.tipo === tipo;
        const matchSector = !sector || n.sector === sector;
        const matchJurisdiccion = !jurisdiccion || n.jurisdiccion === jurisdiccion;
        return matchQuery && matchTipo && matchSector && matchJurisdiccion;
    });
    ...
    return (
        <div className="max-w-4xl mx-auto animate-fade-in">
            <div className="mb-6">
                ...
                <p className="text-sm text-text-tertiary">Búsqueda sobre normativa argentina</p>
            </div>
            ...
            <div className="card p-1.5 mb-3 flex items-center gap-2 focus-within:border-inst-accent transition-colors">
                ...
                <input
                    ...
                    placeholder="Buscar por norma, sector, empresa, keyword..."
            ...
            <p className="text-[11px] text-text-tertiary mb-3">
                {query || tipo || sector || jurisdiccion
                    ? <><span className="font-semibold text-text-primary">{results.length}</span> resultados</>
            ...
            <div className="space-y-2">
                {results.map(norma => (
                    <div
                        key={norma.id}
                        onClick={() => navigate(`/norma/${norma.id}`)}
                        className="card card-hover p-4 cursor-pointer group transition-all"
                    >
                        <div className="flex items-start gap-3">
                            <div
                                className="w-1 h-10 rounded-full shrink-0 mt-0.5"
                                style={{ backgroundColor: TIPO_DOT_COLORS[norma.tipo] }}
                            />
                            <div className="flex-1 min-w-0">
                                <div className="flex flex-wrap items-center gap-2 mb-0.5">
                                    <span className="text-[10px] font-semibold uppercase tracking-wide" style={{ color: TIPO_DOT_COLORS[norma.tipo] }}>
                                        {TIPOS_NORMA[norma.tipo].label} {norma.numero}
                                    </span>
                                    <span className="text-[11px] text-text-tertiary">{norma.fecha}</span>
                                </div>
                                ...
                                <p className="text-[12px] text-text-tertiary line-clamp-1">{norma.resumen}</p>
