...
import {
    Newspaper, BarChart3, Search, Bell, Shield, X, Eye, Landmark
} from 'lucide-react';
...
const Sidebar = ({ open, onClose }) => {
    return (
        <>
            ...
            <aside
                className={`fixed top-0 left-0 h-full w-60 bg-navy-900 z-50 flex flex-col transition-transform duration-200 lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
                {/* Logo */}
                ...
                <nav className="flex-1 py-4 px-3 space-y-0.5 overflow-y-auto">
                    {NAV_ITEMS.map(({ to, label, icon: Icon }) => (
                        <NavLink
                            ...
                            onClick={onClose}
                            className={({ isActive }) =>
                                `flex items-center gap-2.5 px-3 py-2.5 rounded text-[13px] font-medium transition-colors ${isActive
