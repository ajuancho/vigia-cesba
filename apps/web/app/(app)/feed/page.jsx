...
function NormCard({ norma, onClick }) {
  ...
  return (
    <div onClick={onClick} className="card card-hover p-5 cursor-pointer transition-all group animate-fade-in">
      <div className="flex flex-wrap items-center gap-2 mb-3">
        <span className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide border ${TIPO_STYLES[norma.tipo] || TIPO_STYLES.OTRA}`}>
          {tipoMeta.label}
        </span>
        {norma.impacto && (
      ...
      {/* Análisis automático — solo cuando hay resumen IA (Fase 5) */}
      {norma.resumen_ia && (
        <div className="bg-bg-secondary border border-border-light rounded p-3 mb-3">
          ...
          <p className="text-[12px] text-text-secondary leading-relaxed line-clamp-2">{norma.resumen_ia}</p>
        </div>
      )}

      <div className="flex flex-wrap items-center gap-4 text-[11px] text-text-tertiary">
        {norma.organismo && <span className="flex items-center gap-1"><Building2 size={11} /> {norma.organismo}</span>}
        {norma.jurisdiccion && <span className="flex items-center gap-1"><MapPin size={11} /> {norma.jurisdiccion}</span>}
        {norma.sector && <span className="flex items-center gap-1"><Tag size={11} /> {norma.sector}</span>}
  ...
  useEffect(() => {
    ...
    api
      .listNormas({
        ...
        impacto: filterImpacto !== 'todos' ? filterImpacto : undefined,
        limit: 30,
      })
      .then(setData)
      .catch(() => setData({ items: [], total: 0 }))
      .finally(() => setLoading(false));
  }, [filterTipo, filterImpacto]);
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in">
      ...
      <p className="text-[11px] text-text-tertiary mb-3">
        <span className="font-semibold text-text-primary">{data.total}</span> normas
      </p>

      {loading ? (
        <div className="text-center py-16 text-text-tertiary text-sm">Cargando…</div>
      ) : (
        <div className="space-y-3">
          {data.items.map((norma, i) => (
            <div key={norma.id} style={{ animationDelay: `${i * 40}ms` }}>
              <NormCard norma={norma} onClick={() => router.push(`/norma/${norma.id}`)} />
            </div>
          ))}
        </div>
      )}
