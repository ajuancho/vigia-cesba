...
def render_digest(workspace_name: str, items: list[dict]) -> str:
    ...
    rows = "".join(
        ...
        f'{i["keyword"]} · {i["tipo"]} {i.get("numero") or ""}</p>'
        f'<a href="{WEB_BASE_URL}/norma/{i["id"]}" '
        f'style="color:#E8ECF4;font-size:14px;font-weight:600;text-decoration:none">{i["titulo"]}</a>'
        f"</td></tr>"
        if i.get("id")
        else f'<tr><td style="padding:10px 0;border-bottom:1px solid #1f2937">'
        f'<p style="margin:0;color:#E8ECF4;font-size:14px"><strong>{i["keyword"]}</strong> — '
        f'{i["tipo"]} {i.get("numero") or ""}: {i["titulo"]}</p></td></tr>'
        for i in items
    )
    return (
